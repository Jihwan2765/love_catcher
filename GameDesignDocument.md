...
- **보상 유형:**
    1. 레전더리 인형
    2. 일반 인형
    3. 이성 인스타 아이디 (아이디 입력 시)
    4. 사탕
- **기본 확률:** 남성용은 레전더리 10%, 인형 30%, 인스타 40%, 사탕 20%이며, 여성용은 레전더리 10%, 인형 40%, 인스타 30%, 사탕 20%입니다.
- **인스타 미입력:** 남녀별 인스타 미입력 전용 확률(레전더리/인형/사탕)을 개발자 모드에서 별도로 설정합니다. 인스타 지급 대상이 없거나 Firebase 조회에 실패하면 인스타를 제외한 나머지 보상을 기존 비율대로 재추첨합니다.
- **재고:** 일반 인형과 레전더리 인형은 Firebase에서 별도 재고로 관리합니다. 레전더리 재고가 0이면 레전더리를 제외하고 나머지 보상을 비율대로 재추첨합니다.
